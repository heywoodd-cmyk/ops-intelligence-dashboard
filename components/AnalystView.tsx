...
export function AnalystView({
  ...
  // All blocking items merged (bottlenecks + overdue patterns)
  const allBlockingItems = analysis
    ? [...analysis.bottlenecks, ...analysis.overduePatterns]
    : [];
  const displayedBlocking = showAllBlocking
    ? allBlockingItems
    : allBlockingItems.slice(0, LIMIT);
  const hiddenBlocking = allBlockingItems.length - LIMIT;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Error state — soft, non-blocking */}
      {error && !loading && (
        <div className="rounded-xl border border-[#1c2235] bg-[#111520] p-4 flex items-start gap-3">
          <span className="w-1.5 h-1.5 rounded-full bg-[#6b778f] mt-1.5 flex-shrink-0" />
      ...
      {analysis && health && !loading && (
        <div className={`rounded-2xl border ${health.ring} ${health.bg} p-6`}>
          <div className="flex items-center gap-2.5 mb-3">
            <span
              className={`w-2 h-2 rounded-full ${health.dot} animate-pulse`}
            />
            <span
              className={`text-xs font-semibold uppercase tracking-widest ${health.color}`}
            >
              {health.label}
            </span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <QuickStat
          ...
          tooltip="Past due date and not yet done."
        />
        <QuickStat
          label="Blocked"
          ...
          tooltip='Explicitly marked "Blocked" — needs outside help.'
        />
        <QuickStat
          label="Complete"
          ...
          active={false}
          activeClass=""
          tooltip="Share of all tasks marked Done."
        />
      </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <button
                      ...
                      className="text-[10px] text-muted/50 font-mono hover:text-violet-300 transition-colors"
...
function QuickStat({
  ...
  return (
    <div
      className={`rounded-xl border p-4 text-center transition-colors relative group ${
    ...
    >
      ...
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-40 bg-[#1a2030] border border-card-border rounded-lg p-2 text-[11px] text-[#8b96b0] leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 shadow-xl text-center">
        {tooltip}
      </div>
    </div>
  );
