import type { Task as LegacyTask } from "@/app/api/analyze/route";
import {
  normalizeStatus,
  normalizePriority,
  parseDate,
...
// =====================================================================
// LEGACY — retained so existing Phase-1-untouched files still compile
// (SchemaMapper, ValidationSummary, /api/map-schema, current page.tsx).
// Phase 3 cleanup deletes all of this.
// =====================================================================

export const LEGACY_CANONICAL_FIELDS = [
  "task_id",
  "task_name",
  "assignee",
  "status",
...
export type LegacyCanonicalField = (typeof LEGACY_CANONICAL_FIELDS)[number];

export const LEGACY_CANONICAL_STATUSES = [
  "Done",
  "In Progress",
  "Blocked",
  "Not Started",
...
export type LegacyCanonicalStatus = (typeof LEGACY_CANONICAL_STATUSES)[number];

const LEGACY_STATUS_SET = new Set<string>([
  ...LEGACY_CANONICAL_STATUSES,
  "Completed",
]);

export const REQUIRED_FIELDS: LegacyCanonicalField[] = [
  "task_id",
  "task_name",
  "assignee",
  "status",
...
export interface ProposedMapping {
  column_map: Record<LegacyCanonicalField, string | null>;
  status_value_map: Record<string, LegacyCanonicalStatus | null>;
}
...
export function isCanonicalCSV(rows: Record<string, string>[]): boolean {
  ...
  const headers = Object.keys(rows[0]);
  const headersMatch = LEGACY_CANONICAL_FIELDS.every((c) => headers.includes(c));
  if (!headersMatch) return false;
  return rows.every((r) => {
    const v = (r.status || "").trim();
    return v === "" || LEGACY_STATUS_SET.has(v);
  });
}
...
function legacyNormalizeDate(raw: string | undefined | null): {
  value: string;
  parsed: boolean;
  hadInput: boolean;
} {
...
): { tasks: LegacyTask[]; report: ValidationReport } {
  const tasks: LegacyTask[] = [];
  const issues: ValidationIssue[] = [];

  rawData.forEach((row, rowIndex) => {
    const reasons: string[] = [];
    const get = (field: LegacyCanonicalField): string => {
    ...
    let canonicalStatus = "";
    if (rawStatus) {
      if (LEGACY_STATUS_SET.has(rawStatus)) {
        canonicalStatus = rawStatus === "Completed" ? "Done" : rawStatus;
      } else {
        const mapped = mapping.status_value_map[rawStatus];
        if (mapped) canonicalStatus = mapped;
        else reasons.push(`unmapped status value "${rawStatus}"`);
      }
    }
    const created = legacyNormalizeDate(get("created_date"));
    const due = legacyNormalizeDate(get("due_date"));
    const completed = legacyNormalizeDate(get("completed_date"));
    if (!created.parsed)
    ...
    const dropReasons = reasons.filter(
      (r) =>
        r.startsWith("missing required field") ||
        r.startsWith("unmapped status")
    );
...
export function canonicalRowsToTasks(
  rows: Record<string, string>[]
): LegacyTask[] {
  return rows.map((r) => ({
    task_id: r.task_id || "",
